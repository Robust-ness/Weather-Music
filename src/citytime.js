async function cityTime(e) {
    ...
    try {
        ...
        document.querySelector('.weather-submit').innerHTML = `<label>${weather.data[0].city_name}</label><form id="city-form">
              <label>Try Another?</label>
              <input id="city" type="text" required>
              <input class="button" type="submit" value="Click for the weather!">
          </form>`
        document.querySelector('#city-form').addEventListener('submit', cityTime)
        document.querySelector('.now-playing').innerHTML = `Now Playing...<div class='music'></div>`
        document.querySelector('.display-weather').innerHTML = `<div class="condition">Condition<hr><div class="condition-desc">${weather.data[0].weather.description}</div></div><div class="temperature">Temperature<hr><div class="temperature-desc">${weather.data[0].temp + ' &deg;C' + (weather.data[0].temp != weather.data[0].app_temp ? ' | Feels like ' + weather.data[0].app_temp + ' &deg;C' : '')}</div></div><div class="humidity">Humidity<hr><div class="humidity-desc">${weather.data[0].rh + '&percnt;'}</div></div><div class="wind">Wind<hr><div class="wind-desc">${(Math.round(weather.data[0].wind_spd * 2.237 * 100)/100) + ' mph'}</div></div>`
